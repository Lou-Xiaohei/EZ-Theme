    ...
    <div class="dashboard-inner">
      <div class="dashboard-card welcome-card" :class="{'card-animate': !loading.userInfo}">
        <div class="card-header">
          <h2 class="card-title">
            <span v-if="userStats.userEmail && DASHBOARD_CONFIG.showUserEmail" class="user-email">
              {{ userStats.userEmail }}
            </span>
            🎉{{ $t('dashboard.welcome') }}
          </h2>
        ...
        <div class="card-body">
          <p class="">{{ $t('dashboard.welcomeDesc') }}</p>
              ...
              {{ $t('dashboard.currentIPAddress') }}：{{ userLocation }}
            </p>
        </div>
      </div>
          ...
          <div class="card-body">
            <div class="subscription-info">
              ...
              <div class="info-item">
                ...
                <span class="info-value">
                  <span v-if="userPlan.isExpireDatePermanent">{{ $t('dashboard.permanent') }}</span>
                  <span v-else-if="isExpired">
                    已过期<span class="reset-date">({{ userPlan.expireDate }})</span>
                  </span>
                  <span v-else>{{ userPlan.expireDate || $t('dashboard.none') }}</span>
                </span>
              </div>
              <div class="info-item">
              ...
              <div class="info-item" v-if="userPlan.resetDay !== null && userPlan.resetDay !== undefined">
                <span class="info-label">{{ $t('dashboard.nextResetTime') }}</span>
                <span class="info-value">
                  <span>{{ userPlan.resetDay }}天</span>
                  <span class="reset-date">({{ formatResetDate(userPlan.resetDay) }})</span>
                </span>
              </div>
            ...
            <div class="subscription-actions">
              <button class="btn-outline" :class="{
                'btn-active': showClientDownloadCard
              }" @click="toggleClientDownloadCard">
                <IconDownload :size="16" class="btn-icon"/>
                <span class="">{{ $t('dashboard.clientDownload') }}</span>
              </button>
        ...
        <div v-if="showImportCard && userPlan.subscribeUrl" class="dashboard-card import-card">
          <div class="card-header">
            <div class="title-container">
              ...
              <span class="import-subtitle">{{ $t('dashboard.thirdPartyClient') }}</span>
            </div>
          ...
          <div class="card-body">
            ...
            <div v-if="isExpired" class="expired-notice">
              ...
              <div class="expired-content">
                <div class="expired-title">{{ $t('dashboard.expiredTitle') }}</div>
                <div class="expired-desc">{{ $t('dashboard.expiredDesc') }}</div>
              </div>
            </div>
      ...
      <!-- 客户端下载卡片 -->
      <transition name="slide-fade">
        <div v-if="showClientDownloadCard" class="dashboard-card client-download-card">
          <div class="card-header">
            <div class="title-container">
              <h2 class="card-title">{{ $t('dashboard.clientDownload') }}</h2>
              <span class="download-subtitle">{{ $t('dashboard.selectPlatform') }}</span>
            </div>
            <button class="close-btn" @click="showClientDownloadCard = false">
              <span class="close-icon"></span>
            </button>
          </div>
          <div class="card-body">
            <div class="platform-selector">
              <!-- iOS -->
              <div class="platform-card" @click="downloadClient('ios')">
                <IconBrandApple :size="16"/>
                <span>iOS</span>
              </div>
              
              <!-- Android -->
              <div class="platform-card" @click="downloadClient('android')">
                <IconBrandAndroid :size="16"/>
                <span>Android</span>
              </div>
              
              <!-- Windows -->
              <div class="platform-card" @click="downloadClient('windows')">
                <IconBrandWindows :size="16"/>
                <span>Windows</span>
              </div>
              
              <!-- MacOS -->
              <div class="platform-card" @click="downloadClient('macos')">
...
import {CLIENT_CONFIG, DASHBOARD_CONFIG, isXV2B, SITE_CONFIG} from '@/utils/baseConfig';
import {
  IconAlertTriangle,
  IconBox,
  IconBrandAndroid,
...
import {getNotices, getSubscribe, getUserConfig, getUserInfo, getUserStats, setNextPeriod, getConfig} from '@/api/dashboard';
...
export default {
  ...
  setup() {
    ...
    const showClientDownloadCard = ref(false);
    ...
    const downloadClient = async (platform) => {
      try {
        ...
        switch (platform) {
          case 'android':
            downloadUrl = config.android_download_url;
            break;
          case 'windows':
            downloadUrl = config.windows_download_url;
            break;
          case 'macos':
            downloadUrl = config.macos_download_url;
            break;
        }
        ...
        if (downloadUrl) {
          ...
          showToast(t('dashboard.downloadUrlNotAvailable'), 'error');
        }
      } catch (error) {
        console.error('获取下载链接失败:', error);
        showToast(t('dashboard.downloadFailed'), 'error');
      }
    };

    const toggleClientDownloadCard = () => {
      showClientDownloadCard.value = !showClientDownloadCard.value;
      if (showClientDownloadCard.value) {
        showImportCard.value = false;
        nextTick(() => {
          setTimeout(() => {
            const clientDownloadCard = document.querySelector('.client-download-card');
            if (clientDownloadCard) {
              clientDownloadCard.scrollIntoView({
                behavior: 'smooth',
                block: 'center'
              });
            }
    ...
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          // 设置用户信息到store，以便App.vue中的监听器能够检测到变化
          try {
            store.dispatch('setUser', info);
          } catch (e) {
            console.error('Dashboard: 设置用户信息到store失败:', e);
          }
    ...
    const fetchSubscribe = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          } else if (subscribe.transfer_enable) {
            // 如果没有使用流量数据，但有总流量，显示总流量
            userStats.remainingTraffic = formatTraffic(subscribe.transfer_enable);
          } else {
            // 如果没有任何流量数据，显示0
            userStats.remainingTraffic = '0 B';
          }
          if (subscribe.reset_day !== null && subscribe.reset_day !== undefined) {
    ...
    const toggleImportCard = () => {
      ...
      if (showImportCard.value) {
        showClientDownloadCard.value = false;
        nextTick(() => {
          setTimeout(() => {
    ...
    const showDeviceLimit = computed(() => {
      return isXV2B() && DASHBOARD_CONFIG.showOnlineDevicesLimit;
    });
    ...
    // 格式化重置日期
    const formatResetDate = (days) => {
      if (days === null || days === undefined || isNaN(days)) return '';
      
      const now = new Date();
      const resetDate = new Date(now.getTime() + days * 24 * 60 * 60 * 1000);
    ...
    const fetchUserLocation = async () => {
      try {
        ...
        userLocation.value = `${ip} ${location}`;
      } catch (error) {
        console.error('获取用户位置失败:', error);
        userLocation.value = '未知IP 未知地区';
      }
    };
    ...
    return {
      ...
      currentNoticeIndex,
      showImportCard,
      showClientDownloadCard,
      toggleClientDownloadCard,
      showQrCode,
      importToClient,
      goToSupport,
      formatDate,
...
.dashboard-container {
  ...
  .stats-grid {
    ...
    .stats-card {
      position: relative;
      background-color: rgba(var(--card-background-rgb, 255, 255, 255), 0.5);
      border-radius: 16px;
